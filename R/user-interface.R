...
# view_rg: c(from, to)
# coord_rg: c(min, max)
# zoom_allow_rg: c(zoom_in, zoom_out)
tnt_board <- function (view_rg, coord_rg, zoom_allow_rg,
                       width = 700, allow_drag = TRUE,
                       with_locinfo = TRUE, axis_at = c("top","bottom","none")) {
   # jc.view_rg <- jc_rg(view_rg, byname = TRUE, use = "from/to")
...
#' @export
print.tnt_board <- function(tntdef) {
    TnT(tntdef)
    invisible(tntdef)
}
